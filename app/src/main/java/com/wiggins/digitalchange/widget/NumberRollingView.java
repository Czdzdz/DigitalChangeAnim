...
public class NumberRollingView extends TextView {
    ...
    private long nowNum;//记录每帧增加后的数字
    private long finalNum;//目标数字（最终的数字）
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case NUM_TYPE://普通数字滚动
                    ...
                    break;
                case NUM_STRING_TYPE://金额文本显示
                    // 更新显示的内容
                    if (useCommaFormat) {
                        NumberRollingView.this.setText(StringUtil.addUnit(nowNum));
                    } else {
                        NumberRollingView.this.setText(StringUtil.addUnit(nowNum));
                    }
                    ...
                    if (nowNum < finalNum) {
                        ...
                        //已经达到目标的数字，显示最终的内容
                        if (useCommaFormat) {
                            NumberRollingView.this.setText(StringUtil.addUnit(finalNum));
                        } else {
                            NumberRollingView.this.setText(StringUtil.addUnit(finalNum));
                        }
                    }
                    break;
                case NUM_STRING_TYPE_WITH_STYLE://金额文本显示
                    // 更新显示的内容
                    if (useCommaFormat) {
                        NumberRollingView.this.setText(StringUtil.addUnitWithStyle(nowNum));
                    } else {
                        NumberRollingView.this.setText(StringUtil.addUnitWithStyle(nowNum));
                    }
                    ...
                    if (nowNum < finalNum) {
                        //如果当前记录的数字小于目标数字，即还没达到目标数字，继续递增
                        Message msg2 = handler.obtainMessage();
                        msg2.what = NUM_STRING_TYPE_WITH_STYLE;
                        msg2.obj = msg.obj;
                        // 继续发送通知改变UI
                        handler.sendMessage(msg2);
                    } else {
                        //已经达到目标的数字，显示最终的内容
                        if (useCommaFormat) {
                            NumberRollingView.this.setText(StringUtil.addUnitWithStyle(finalNum));
                        } else {
                            NumberRollingView.this.setText(StringUtil.addUnitWithStyle(finalNum));
                        }
                    }
     ...
     */
    public void setContent(String str, boolean filter) {
        ...
        if (runWhenChange) {
            ...
            if (preStr.equals(str) && filter == true) {
                //如果两次内容一致，则不做处理
                return;
            }
    ...
    private void useAnimByType(String str) {
        if (textType == MONEY_TYPE) {
            ...
            startNumAnim(str);
        } else if (textType == NUM_STRING_TYPE) {
            startNumTextAnim(str);
        } else if (textType == NUM_STRING_TYPE_WITH_STYLE) {
    ...
    /**
     * @param numStr
     * @Description 开启数字文本显示滚动 带样式
     */
    private void startNumTextStyleAnim(String numStr) {
        // 如果传入的数字已经格式化了，则将包含的符号去除
        String num = numStr.replace(",", "").replace("-", "");
        try {
            finalNum = Long.parseLong(num);
            ...
            threadPool.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    long temp = finalNum / frameNum;
                    msg.what = NUM_STRING_TYPE_WITH_STYLE;
                    msg.obj = temp;
                    // 发送消息改变UI
    ...
    /**
     * @param numStr
     * @Description 开启数字文本显示滚动
     */
    private void startNumTextAnim(String numStr) {
// 如果传入的数字已经格式化了，则将包含的符号去除
        String num = numStr.replace(",", "").replace("-", "");
        try {
            finalNum = Long.parseLong(num);
